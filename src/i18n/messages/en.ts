export const en = {
  "common.loading": "Loading…",
  "common.listSep": ", ",
  "tab.todo": "Todo",
  "tab.reminder": "Reminders",
  "tab.timer": "Timer",
  "nav.profile": "Mine",

  "quickAdd.placeholder": "Add in one sentence…",
  "quickAdd.add": "Add",
  "quickAdd.unrecognized": "Not recognized as reminder / timer / todo — try rephrasing",
  "quickAdd.added": "Added: {items}",
  "quickAdd.errNetwork": "Network issue — check your connection and retry",
  "quickAdd.errRateLimited": "Too many requests — please try again later",
  "quickAdd.errGeneric": "Couldn't add — please try again later",
  "quickAdd.aiFallback": "AI unavailable — parsed locally",
  "quickAdd.addedDaily": "Added daily reminder",

  "todo.empty": "No todos yet",
  "todo.completeAria": "Complete \"{content}\"",
  "todo.editAria": "Edit content",
  "action.edit": "Edit",
  "action.delete": "Delete",
  "action.save": "Save",
  "action.cancel": "Cancel",
  "err.actionFailed": "Action failed",
  "err.deleteFailed": "Delete failed",
  "err.saveFailed": "Save failed",
  "err.restoreFailed": "Restore failed",

  "reminder.empty": "No pending reminders",
  "reminder.pendingHeader": "Pending reminders",
  "reminder.dailyHeader": "Daily reminders",
  "reminder.dailyAt": "Every day at {time}",

  "timer.pickOne": "Pick a timer",
  "timer.preset.pomodoro": "Pomodoro",
  "timer.preset.rule5217": "52/17 Rule",
  "timer.preset.rule90": "90-Min Rule",
  "timer.preset.shortBreak": "Short break",
  "timer.preset.longBreak": "Long break",
  "timer.pomodoroTechnique": "Pomodoro technique",
  "timer.rule5217Technique": "52/17 method",
  "timer.rule90Technique": "90-minute method",
  "timer.customFocusTechnique": "Custom focus",
  "timer.newTimer": "New timer",
  "timer.nameLabel": "Name",
  "timer.namePlaceholder": "Timer name",
  "timer.kindCountdown": "Countdown",
  "timer.kindFocus": "Focus",
  "timer.durationLabel": "Minutes",
  "timer.workLabel": "Work (min)",
  "timer.breakLabel": "Break (min)",
  "timer.cyclesLabel": "Cycles",
  "timer.invalidInput": "Enter a name and valid duration",
  "timer.deleteCustomAria": "Delete {name}",
  "timer.minutes": "{n} min",
  "timer.setupSubtitle": "{work} min work · {short} min break · long break every 4th ({long} min)",
  "timer.setupSubtitleSimple": "{work} min work · {break} min break",
  "timer.chooseCycles": "Number of cycles",
  "timer.decreaseAria": "Decrease",
  "timer.increaseAria": "Increase",
  "timer.totalMinutes": "{n} min total",
  "timer.estimatedEnd": "Ends around {time}",
  "timer.start": "Start",
  "timer.back": "← Back",
  "timer.cycleProgress": "Round {current} of {total}",
  "timer.workRunning": "Working",
  "timer.workEnded": "Work done",
  "timer.breakRunning": "Break",
  "timer.breakEnded": "Break over",
  "timer.longRunning": "Long break",
  "timer.longEnded": "Long break over",
  "timer.allDone": "All done — great work!",
  "timer.finish": "Finish",
  "timer.startBreak": "Start break ({min} min)",
  "timer.startLongBreak": "Start long break ({min} min)",
  "timer.startNextRound": "Start round {n}",
  "timer.pause": "Pause",
  "timer.resume": "Resume",
  "timer.resetPhase": "Reset phase",
  "timer.reset": "Reset",
  "timer.endMethod": "End {name}",
  "timer.paused": "Paused",
  "timer.hintSession": "Timing continues on other tabs; you'll confirm when it's up.",
  "timer.hintOneShot": "Timing continues on other tabs; you'll get a system notification.",

  "widget.work": "Work",
  "widget.break": "Break",
  "widget.title": "Round {current}/{total} · {phase}",
  "widget.allDone": "All done",
  "widget.phaseDone": "Phase done",
  "widget.next": "Next",
  "widget.openAria": "Open timer",
  "widget.collapseAria": "Collapse timer",
  "widget.expandAria": "Expand timer",

  "profile.title": "Mine",
  "profile.backAria": "Back",
  "profile.synced": "Synced",
  "profile.localMode": "Local mode",
  "profile.notSignedIn": "Not signed in",
  "profile.dataLocalOnly": "Stored on this device only",
  "profile.signInToSync": "Sign in to sync across devices",
  "profile.localHint": "Sign in to sync your todos and reminders to your account and see them on other devices. You can keep using the app without signing in — data stays in this browser.",
  "profile.signOut": "Sign out",
  "profile.segTodos": "Completed todos",
  "profile.segReminders": "Past reminders",
  "profile.noDoneTodos": "No completed todos yet",
  "profile.noReminders": "No past reminders yet",
  "profile.restore": "Restore",
  "profile.restoreAria": "Restore \"{content}\" to active",
  "profile.deleteAria": "Delete \"{content}\"",
  "profile.language": "Language",
  "profile.langSystem": "System default",
  "profile.aiSection": "AI parsing (DeepSeek)",
  "profile.aiEnabled": "Enabled",
  "profile.aiNotSet": "Not set",
  "profile.aiKeyPlaceholder": "Paste your DeepSeek API key",
  "profile.aiSave": "Save",
  "profile.aiSaving": "Checking…",
  "profile.aiClear": "Clear",
  "profile.aiValid": "✓ Enabled",
  "profile.aiInvalid": "✗ Invalid key",
  "profile.aiNetworkErr": "✗ Network error, try again",
  "profile.aiHint": "Your key is stored locally and requests go directly to DeepSeek, never through our server. Get a key at platform.deepseek.com.",
  "profile.clipboardSection": "Clipboard",
  "todo.pasteImage": "Paste image",
  "todo.fromClipboard": "From clipboard",
  "todo.remindLabel": "Remind",
  "todo.clearRemind": "Clear",
  "todo.imageMax": "At most {max} images",
  "todo.imageTooLarge": "Image too large (25MB max)",
  "todo.noImageInClipboard": "No image in the clipboard",
  "todo.imageAddFailed": "Failed to add image",
  "todo.viewImageAria": "View image {n}",
  "todo.removeImageAria": "Remove image {n}",
  "todo.imagePreviewAria": "Image preview",
  "todo.imagePickTitle": "Pick from clipboard",
  "todo.imagePickEmpty": "No images on the clipboard yet",

  "todo.categoryAll": "All",
  "todo.categoryNone": "Uncategorized",
  "todo.categoryLabel": "Category",
  "todo.categoryFilterAria": "Filter by category",
  "todo.manageCategories": "Manage categories",
  "todo.doneManaging": "Done",
  "todo.newCategory": "New category",
  "todo.addCategory": "Add",
  "todo.noCategories": "No categories yet",
  "todo.categoryNameTaken": "A category with that name already exists",
  "todo.renameCategoryAria": "Rename \"{name}\"",
  "todo.deleteCategoryAria": "Delete \"{name}\"",
  "todo.deleteCategoryConfirm": "Delete? Its todos become uncategorized.",
  "profile.soundSection": "Notification sound",
  "profile.soundLabel": "Play a chime when due",
  "profile.soundHint": "A short chime plays alongside the notification. Reminders and timers use different tones; adjust loudness with your system volume.",

  "login.title": "Sign in to Remind",
  "login.subtitle": "Sign in to sync across devices; you can also use the app locally without signing in.",
  "login.idPlaceholder": "Username or email",
  "login.pwPlaceholder": "Password",
  "login.submit": "Sign in",
  "login.submitting": "Signing in…",
  "login.useLocal": "Skip — use locally",
  "login.register": "No account? Register on the web",
  "login.failed": "Sign in failed",

  "notify.reminderTitle": "Reminder",
  "notify.todoRemindTitle": "Todo",
  "notify.timeUp": "Time's up",
  "notify.timerEnded": "\"{name}\" finished",
  "notify.breakTitle": "Time for a break",
  "notify.breakBody": "\"{name}\" done — open the panel to start your break",
  "notify.allDoneTitle": "All done 🎉",
  "notify.allDoneBody": "Your {name} session is complete",
  "notify.breakOverTitle": "Break over",
  "notify.breakOverBody": "Open the panel to start the next round",

  "tab.clipboard": "Clipboard",
  "clip.searchPlaceholder": "Search clipboard…",
  "clip.filterAll": "All",
  "clip.filterText": "Text",
  "clip.filterImage": "Images",
  "clip.pasteZone": "Paste or drop text / image (Ctrl+V)",
  "clip.addFromClipboard": "Add from clipboard",
  "clip.groupPinned": "Pinned",
  "clip.groupToday": "Today",
  "clip.groupEarlier": "Earlier",
  "clip.empty": "Nothing copied yet",
  "clip.noMatch": "No matching items",
  "clip.expand": "Show full text",
  "clip.collapse": "Show less",
  "clip.sourceManual": "Manual",
  "clip.copyTitle": "Copy again",
  "clip.pinTitle": "Pin",
  "clip.unpinTitle": "Unpin",
  "clip.deleteTitle": "Delete",
  "clip.copied": "Copied to clipboard",
  "clip.copiedImage": "Image copied to clipboard",
  "clip.saved": "Saved from clipboard",
  "clip.limitLabel": "Keep latest",
  "clip.limitUnit": "items",
  "clip.copyFailed": "Copy failed",
  "clip.readFailed": "Couldn't read clipboard",
  "clip.saveImageFailed": "Couldn't save image",
  "clip.menuSaveImage": "Copy image to Remind clipboard",
  "clip.actionFailed": "Action failed",
  "clip.imageTooLarge": "Image too large to save",
  "clip.autoCaptureLabel": "Auto-capture copied text",
  "notepad.title": "Notes",
  "notepad.placeholder": "Jot something down…",
  "notepad.saved": "Saved",
  "notepad.charCount": "{n} chars",
  "notepad.expand": "Expand",
  "notepad.collapse": "Collapse",
  "notepad.toggle": "Toggle notes",
  "notepad.addToClipboard": "Add to clipboard",
  "notepad.addedToClipboard": "Added to clipboard",
  "shot.button": "Screenshot",
  "shot.hint": "Drag to select an area. It will be copied to your clipboard.",
  "shot.menuTitle": "Take a screenshot",
  "shot.copied": "Screenshot copied to clipboard",
  "shot.copyFailed": "Could not copy. Click the page, then try again.",
  "shot.copyUnavailable": "This page does not allow writing to the clipboard.",
  "shot.unsupportedPage": "Screenshots aren't available on this page.",
  "shot.permissionDenied": "Permission denied. Use the keyboard shortcut or the page's right-click menu instead.",
  "shot.startFailed": "Couldn't start the screenshot. Please try again, or reopen the side panel.",
  "shot.settingsSection": "Screenshot",
  "shot.entrySidePanel": "Button in the side panel",
  "shot.entryContextMenu": "Page right-click menu",
  "shot.entryShortcut": "Keyboard shortcut (Ctrl+Shift+S)",
  "shot.shortcutHint": "Chrome does not allow unregistering a shortcut. Turning it off makes it do nothing; the key combination still appears on chrome://extensions/shortcuts.",
  "shot.toolSelect": "Select area",
  "shot.toolMosaic": "Pixelate",
  "shot.toolRect": "Rectangle",
  "shot.toolArrow": "Arrow",
  "shot.toolText": "Text",
  "shot.colorRed": "Red",
  "shot.colorYellow": "Yellow",
  "shot.colorGreen": "Green",
  "shot.colorBlue": "Blue",
  "shot.brushSmall": "Small",
  "shot.brushMedium": "Medium",
  "shot.brushLarge": "Large",
  "shot.undo": "Undo",
} as const;

export type MessageKey = keyof typeof en;
