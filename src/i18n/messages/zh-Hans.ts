import type { en } from "./en";

export const zhHans: Record<keyof typeof en, string> = {
  "common.loading": "加载中…",
  "common.listSep": "、",
  "tab.todo": "待办",
  "tab.reminder": "提醒",
  "tab.timer": "计时",
  "nav.profile": "我的",

  "quickAdd.placeholder": "一句话添加…",
  "quickAdd.add": "添加",
  "quickAdd.unrecognized": "未识别为提醒 / 计时 / 待办，换个说法试试",
  "quickAdd.added": "已添加：{items}",
  "quickAdd.errNetwork": "网络不给力，请检查连接后重试",
  "quickAdd.errRateLimited": "操作太频繁了，请稍后再试",
  "quickAdd.errGeneric": "没能添加成功，请稍后再试",
  "quickAdd.aiFallback": "AI 不可用，已用本地解析",
  "quickAdd.addedDaily": "已添加每日提醒",

  "todo.empty": "暂无待办",
  "todo.completeAria": "完成「{content}」",
  "todo.editAria": "编辑内容",
  "action.edit": "编辑",
  "action.delete": "删除",
  "action.save": "保存",
  "action.cancel": "取消",
  "err.actionFailed": "操作失败",
  "err.deleteFailed": "删除失败",
  "err.saveFailed": "保存失败",
  "err.restoreFailed": "恢复失败",

  "reminder.empty": "暂无待触发的提醒",
  "reminder.pendingHeader": "待触发的提醒",
  "reminder.dailyHeader": "每日提醒",
  "reminder.dailyAt": "每天 {time}",

  "timer.pickOne": "选择一个计时",
  "timer.preset.pomodoro": "番茄钟",
  "timer.preset.rule5217": "52/17 计时",
  "timer.preset.rule90": "90 分钟计时",
  "timer.preset.shortBreak": "短休息",
  "timer.preset.longBreak": "长休息",
  "timer.pomodoroTechnique": "番茄工作法",
  "timer.rule5217Technique": "52/17 工作法",
  "timer.rule90Technique": "90 分钟工作法",
  "timer.customFocusTechnique": "自定义专注",
  "timer.newTimer": "新建计时器",
  "timer.nameLabel": "名称",
  "timer.namePlaceholder": "计时器名称",
  "timer.kindCountdown": "倒计时",
  "timer.kindFocus": "专注法",
  "timer.durationLabel": "时长(分钟)",
  "timer.workLabel": "工作(分钟)",
  "timer.breakLabel": "休息(分钟)",
  "timer.cyclesLabel": "默认循环次数",
  "timer.invalidInput": "请填写名称和有效时长",
  "timer.deleteCustomAria": "删除{name}",
  "timer.minutes": "{n} 分钟",
  "timer.setupSubtitle": "{work} 分钟工作 · {short} 分钟休息 · 每 4 轮长休息 {long} 分钟",
  "timer.setupSubtitleSimple": "{work} 分钟工作 · {break} 分钟休息",
  "timer.chooseCycles": "选择循环次数",
  "timer.decreaseAria": "减少",
  "timer.increaseAria": "增加",
  "timer.totalMinutes": "共 {n} 分钟",
  "timer.estimatedEnd": "预计 {time} 结束",
  "timer.start": "开始",
  "timer.back": "← 返回",
  "timer.cycleProgress": "第 {current} 轮 / 共 {total} 轮",
  "timer.workRunning": "工作中",
  "timer.workEnded": "工作结束",
  "timer.breakRunning": "休息中",
  "timer.breakEnded": "休息结束",
  "timer.longRunning": "长休息中",
  "timer.longEnded": "长休息结束",
  "timer.allDone": "全部完成，辛苦了！",
  "timer.finish": "完成",
  "timer.startBreak": "开始休息（{min} 分钟）",
  "timer.startLongBreak": "开始长休息（{min} 分钟）",
  "timer.startNextRound": "开始第 {n} 轮工作",
  "timer.pause": "暂停",
  "timer.resume": "继续",
  "timer.resetPhase": "重置本阶段",
  "timer.reset": "重置",
  "timer.endMethod": "结束{name}",
  "timer.paused": "已暂停",
  "timer.hintSession": "切换到其他页面时计时继续，到点后等待确认。",
  "timer.hintOneShot": "切换到其他页面时计时继续，到点会有系统通知。",

  "widget.work": "工作",
  "widget.break": "休息",
  "widget.title": "第 {current}/{total} 轮 · {phase}",
  "widget.allDone": "全部完成",
  "widget.phaseDone": "阶段完成",
  "widget.next": "下一步",
  "widget.openAria": "打开计时",
  "widget.collapseAria": "收起计时",
  "widget.expandAria": "展开计时",

  "profile.title": "我的",
  "profile.backAria": "返回",
  "profile.synced": "已同步",
  "profile.localMode": "本地模式",
  "profile.notSignedIn": "未登录",
  "profile.dataLocalOnly": "数据仅存本机",
  "profile.signInToSync": "登录以多设备同步",
  "profile.localHint": "登录后可把本地的待办与提醒同步到账号，换设备也能看到。不登录也能正常使用，数据保存在此浏览器。",
  "profile.signOut": "退出登录",
  "profile.segTodos": "已完成待办",
  "profile.segReminders": "历史提醒",
  "profile.noDoneTodos": "还没有已完成的待办",
  "profile.noReminders": "还没有历史提醒",
  "profile.restore": "恢复",
  "profile.restoreAria": "恢复「{content}」为未完成",
  "profile.deleteAria": "删除「{content}」",
  "profile.language": "语言",
  "profile.langSystem": "系统默认",
  "profile.aiSection": "AI 智能解析 (DeepSeek)",
  "profile.aiEnabled": "已启用",
  "profile.aiNotSet": "未设置",
  "profile.aiKeyPlaceholder": "粘贴你的 DeepSeek API Key",
  "profile.aiSave": "保存",
  "profile.aiSaving": "校验中…",
  "profile.aiClear": "清除",
  "profile.aiValid": "✓ 已启用",
  "profile.aiInvalid": "✗ Key 无效",
  "profile.aiNetworkErr": "✗ 网络错误，请重试",
  "profile.aiHint": "Key 仅存本地，请求直连 DeepSeek，不经过我方服务器。可在 platform.deepseek.com 获取 Key。",
  "profile.clipboardSection": "剪贴板",
  "todo.pasteImage": "粘贴图片",
  "todo.fromClipboard": "从剪贴板选",
  "todo.remindLabel": "提醒",
  "todo.clearRemind": "清除",
  "todo.imageMax": "最多 {max} 张图片",
  "todo.imageTooLarge": "图片过大（上限 25MB）",
  "todo.noImageInClipboard": "剪贴板里没有图片",
  "todo.imageAddFailed": "添加图片失败",
  "todo.viewImageAria": "查看第 {n} 张图片",
  "todo.removeImageAria": "删除第 {n} 张图片",
  "todo.imagePreviewAria": "图片预览",
  "todo.imagePickTitle": "从剪贴板选一张",
  "todo.imagePickEmpty": "剪贴板里还没有图片",

  "todo.categoryAll": "全部",
  "todo.categoryNone": "未分类",
  "todo.categoryLabel": "分类",
  "todo.categoryFilterAria": "按分类筛选",
  "todo.manageCategories": "管理分类",
  "todo.doneManaging": "完成",
  "todo.newCategory": "新分类",
  "todo.addCategory": "添加",
  "todo.noCategories": "还没有分类",
  "todo.categoryNameTaken": "已有同名分类",
  "todo.renameCategoryAria": "重命名「{name}」",
  "todo.deleteCategoryAria": "删除「{name}」",
  "todo.deleteCategoryConfirm": "删除？其中的待办会变成未分类。",
  "profile.soundSection": "声音提醒",
  "profile.soundLabel": "到时播放提示音",
  "profile.soundHint": "弹通知的同时会响一声短提示音，提醒与计时音色不同。音量请用系统音量调节。",

  "login.title": "登录「记得」",
  "login.subtitle": "登录后数据可在多设备间同步；不登录也能在本机使用。",
  "login.idPlaceholder": "用户名或邮箱",
  "login.pwPlaceholder": "密码",
  "login.submit": "登录",
  "login.submitting": "登录中…",
  "login.useLocal": "先不登录，本地使用",
  "login.register": "没有账号？去网页版注册",
  "login.failed": "登录失败",

  "notify.reminderTitle": "提醒",
  "notify.todoRemindTitle": "待办",
  "notify.timeUp": "时间到",
  "notify.timerEnded": "「{name}」已结束",
  "notify.breakTitle": "该休息了",
  "notify.breakBody": "「{name}」完成，打开面板开始休息",
  "notify.allDoneTitle": "全部完成 🎉",
  "notify.allDoneBody": "本轮{name}已全部完成",
  "notify.breakOverTitle": "休息结束",
  "notify.breakOverBody": "打开面板开始下一轮工作",

  "tab.clipboard": "剪贴板",
  "clip.searchPlaceholder": "搜索剪贴板内容…",
  "clip.filterAll": "全部",
  "clip.filterText": "文字",
  "clip.filterImage": "图片",
  "clip.pasteZone": "粘贴或拖入文字 / 图片（Ctrl+V）",
  "clip.addFromClipboard": "从剪贴板添加",
  "clip.groupPinned": "置顶",
  "clip.groupToday": "今天",
  "clip.groupEarlier": "更早",
  "clip.empty": "还没有复制记录",
  "clip.noMatch": "没有匹配的记录",
  "clip.expand": "展开全文",
  "clip.collapse": "收起",
  "clip.sourceManual": "手动",
  "clip.copyTitle": "重新复制",
  "clip.pinTitle": "置顶",
  "clip.unpinTitle": "取消置顶",
  "clip.deleteTitle": "删除",
  "clip.copied": "已复制到剪贴板",
  "clip.copiedImage": "图片已复制到剪贴板",
  "clip.saved": "已从剪贴板添加",
  "clip.limitLabel": "保留最近",
  "clip.limitUnit": "条",
  "clip.copyFailed": "复制失败",
  "clip.readFailed": "读取剪贴板失败",
  "clip.saveImageFailed": "图片保存失败",
  "clip.menuSaveImage": "复制图片到「记得」剪贴板",
  "clip.actionFailed": "操作失败",
  "clip.imageTooLarge": "图片过大，无法保存",
  "clip.autoCaptureLabel": "自动捕获复制的文字",
  "notepad.title": "记事本",
  "notepad.placeholder": "随手写点什么……",
  "notepad.saved": "已自动保存",
  "notepad.charCount": "{n} 字",
  "notepad.expand": "放大",
  "notepad.collapse": "收起",
  "notepad.toggle": "展开/折叠",
  "notepad.addToClipboard": "添加到剪贴板",
  "notepad.addedToClipboard": "已添加到剪贴板",
  "shot.button": "截图",
  "shot.hint": "拖动框选一块区域，结果会复制到系统剪贴板。",
  "shot.menuTitle": "截图",
  "shot.copied": "截图已复制到剪贴板",
  "shot.copyFailed": "复制失败，点一下页面再试一次",
  "shot.copyUnavailable": "当前页面无法写入剪贴板",
  "shot.unsupportedPage": "当前页面不支持截图",
  "shot.permissionDenied": "未授权，请改用快捷键或页面右键菜单",
  "shot.startFailed": "截图未能启动，请重试，或重新打开侧边栏",
  "shot.settingsSection": "截图",
  "shot.entrySidePanel": "侧边栏按钮",
  "shot.entryContextMenu": "页面右键菜单",
  "shot.entryShortcut": "键盘快捷键（Ctrl+Shift+S）",
  "shot.shortcutHint": "Chrome 不允许注销快捷键。关掉后按键不再有反应，但组合键仍会列在 chrome://extensions/shortcuts 里。",
  "shot.toolSelect": "选区",
  "shot.toolMosaic": "马赛克",
  "shot.toolRect": "矩形框",
  "shot.toolArrow": "箭头",
  "shot.toolText": "文字",
  "shot.colorRed": "红色",
  "shot.colorYellow": "黄色",
  "shot.colorGreen": "绿色",
  "shot.colorBlue": "蓝色",
  "shot.brushSmall": "细",
  "shot.brushMedium": "中",
  "shot.brushLarge": "粗",
  "shot.undo": "撤销",
};
