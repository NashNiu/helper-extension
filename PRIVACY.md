...
The free, default experience parses your one‑line input **entirely on your device** using built‑in rules — no text is sent anywhere for parsing. Any server‑side AI parsing applies only to signed‑in accounts, and processes only the text you submit for that purpose.
...
免费的默认体验会**完全在你的设备上**用内置规则解析你输入的一句话,不会把文本发送到任何地方进行解析。服务器端的 AI 解析仅适用于已登录账号,且只处理你为此提交的文本。
