...
**Clipboard auto-capture (optional, on by default).** When enabled, text you copy on web pages is saved to your local clipboard history so you can find it later, and it is stored only on your device. Only text you select and copy is captured — programmatic copies (for example from a password manager) do not trigger it. You can turn auto-capture off anytime in the Clipboard tab.
...
- **content script (all sites)** — when clipboard auto-capture is enabled, reads the text you copy on a page so it can be saved to your local clipboard history. You can disable this in the Clipboard tab; nothing is read or sent when it is off.
...
**剪贴板自动捕获(可选,默认开启)。** 开启后,你在网页上复制的文字会被保存到本地剪贴板历史,便于稍后查找,且仅保存在你的设备上。只有你主动选中并复制的文字才会被捕获——程序化复制(例如密码管理器)不会触发。你可以随时在剪贴板页关闭自动捕获。
...
- **内容脚本(所有网站)** —— 开启剪贴板自动捕获后,读取你在网页复制的文字以存入本地剪贴板历史。可在剪贴板页关闭;关闭时不读取、不发送任何内容。
