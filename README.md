...
| 功能 | 说明 |
|------|------|
| 待办 | 任务清单,分页 + 触底加载;编辑态可粘贴或从剪贴板选图作为附件(每条最多 9 张,长边超过 1600px 才降采样),缩略图点开全屏预览 |
| 提醒 | 单次提醒到点系统通知;后台闹钟触发 |
| 每日提醒 | 自然语言创建每天定时重复的提醒(如「每天8点提醒我喝水」「每晚10点吃药」),到点系统通知,直到手动删除;在「提醒」页顶部单独区域管理 |
| 计时 | 经典番茄钟循环(工作/休息、每 4 轮长休息)、暂停/继续/重置、预计结束、悬浮小组件;工具栏图标徽标显示剩余分钟与当前阶段(工作/休息/暂停/待确认) |
| 到点提示音 | 提醒与计时到点时播放本地合成的短提示音(离屏文档播放),并压掉系统通知自带的提示音以免重叠;可在「我的」里关闭 |
| 剪贴板 | 收藏文字/图片,置顶、搜索;右键「保存图片」,面板内一键「从剪贴板添加」 |
| 截图 | 键盘快捷键(Ctrl+Shift+S)/ 页面右键菜单 / 侧边栏按钮三个入口,均可在「我的」独立开关;冻结当前页面后拖动框选,可在选区内加标注——马赛克、矩形框、箭头、文字(文字能点回去改内容、也能拖动位置),四色可选、粗细三档、支持撤销;点确认才写入系统剪贴板,取消则整张丢弃(不落库、不上传) |
...
```
src/
  background/     # Service Worker:chrome.alarms 心跳/提醒/每日/计时闹钟、通知、剪贴板、纯逻辑(可单测)
...
- **每日提醒**(`daily:{id}`):独立于登录态的本地重复提醒,存于 `chrome.storage.local`;到点发通知后自动重排次日,仅在浏览器运行时触发(错过当天不补,不做补发)。心跳只为「缺失」的每日闹钟补建,不会重排即将到点的闹钟。
...
| `alarms` | 提醒 / 计时到点调度 |
| `notifications` | 系统通知 |
| `offscreen` | 到点提示音(MV3 service worker 无 DOM,无法直接用 Web Audio) |
| `storage` / `unlimitedStorage` | 本地数据(含剪贴板图片、待办附图) |
| `contextMenus` | 右键「保存图片」到剪贴板;截图入口 |
| `clipboardRead` / `clipboardWrite` | 面板内「从剪贴板添加」/「复制」;截图结果写入系统剪贴板 |
| `activeTab` | 快捷键 / 页面右键菜单触发截图时,临时获取当前标签页访问权限 |
| `commands` | 截图键盘快捷键(Ctrl+Shift+S) |
| `host_permissions` | 后端域名(登录时同步,当前 UI 未开放);`api.deepseek.com`(填入 Key 后 AI 解析) |
| `optional_host_permissions`(`<all_urls>`) | 仅在侧边栏截图按钮被点击时申请(该入口没有 `activeTab` 手势),可在扩展详情页随时撤销 |
