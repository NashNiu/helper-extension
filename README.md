...
Chrome 侧边栏(Side Panel)扩展:一句话搞定提醒、计时与待办。**不登录也能用**——数据存在本地;登录后可与账号同步。
...
| 一句话添加 | 顶部输入框自然语言添加,自动分流到提醒/计时/待办 |
...
未登录时的「一句话添加」通过后端**匿名接口**(`/api/public/*`,带限流)完成 AI 解析,AI 密钥始终留在服务端。
